...
def descargar_eps_trimestral(ticker: str) -> pd.Series:
    """
    Descarga el EPS reportado por trimestre (Reported EPS) via yfinance.
    Requiere conexion a internet; se corre en la maquina del usuario.
    """
    import yfinance as yf

    tk = yf.Ticker(ticker)
    df = tk.get_earnings_dates(limit=80)  # trae historico + estimados futuros
    if df is None or df.empty:
        raise ValueError(f"No se obtuvo historial de earnings para {ticker}.")

    df = df.dropna(subset=["Reported EPS"])
    df = df.sort_index()
    eps = df["Reported EPS"].astype(float)
